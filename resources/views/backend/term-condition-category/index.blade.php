                    ...
                    <div class="dt-action-buttons text-end">
                        <div class="dt-buttons">
                           {{-- <button type="button" class="dt-button create-new btn btn-custom-primary"
                                    ...
                                    data-bs-target="#importExcelFormModal">
                                        <span>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"><i
                            class="ti ti-x close-button-icon"></i></button>
                    <a href="{{ asset('assets/sample_files/Terms-Sample-File.xlsx') }}" class="btn btn-custom-primary mr-3">Sample File</a>
                </div>
                <div class="modal-body">
...
@push('scripts')
    ...
    <script>
        function showFormModal(id = '') {
            ...
            $('#categoryForm').attr('action', '{{route('term-condition-categories.store')}}');
            ...
            if (id != '') {
                ...
                $.ajax({
                    type: 'post',
                    url: '{{route('term-condition-categories.edit')}}',
                    data: {
                        _token: '{{csrf_token()}}',
                        id: id,
                    },
                    success: function (response) {
                        if (response.status == true) {
                            $('#categoryForm').attr('action', '{{route('term-condition-categories.update','uuid')}}'.replace('uuid', response.data.uuid));
        ...
        function submitTermsImport() {
            ...
            $.ajax({
                type: 'post',
                url: '{{--{{route('term-condition-categories.import')}}--}}',
                data: formData,
                processData: false,
                contentType: false,
                success: function (response) {
