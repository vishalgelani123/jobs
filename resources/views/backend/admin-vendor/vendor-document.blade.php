                            ...
                            <div class="card-body">
                                <div class="info-container">
                                    <div class="d-flex justify-content-between">
                                        <h5 class="card-title mb-0">Vendor Documents</h5>
                                    </div>
                                    ...
                                    <form method="POST"
                                          action="{{ route('vendors.vendor.document.store',$vendor) }}"
                                                            ...
                                                            @foreach($vendorDocDetails as $key => $vendorDocDetail)
                                                                <tr>
                                                                    ...
                                                                    <td class="text-center">@if(array_key_exists($vendorDocDetail->vendor_doc_type_id,$uploadedVendorDocs))
                                                                            <a target="_blank"
                                                                               href="{{asset('vendor_documents/'.$uploadedVendorDocs[$vendorDocDetail->vendor_doc_type_id])}}">{{$uploadedVendorDocs[$vendorDocDetail->vendor_doc_type_id]}}</a>
                                                                        @else
...
@push('scripts')
    <script>
        function changeDocumentStatus(vendorDocId, status) {
            $.ajax({
                type: 'post',
                url: '{{ route('vendors.status.change',$vendor) }}',
                data: {
                    _token: '{{csrf_token()}}',
