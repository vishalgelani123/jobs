...
class TermConditionDataTable extends DataTable
{
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('description', function ($query) {
                return '<a class="btn btn-primary btn-sm" href="' . route('term-conditions.category.description', [$query->category,$query]) . '">View</a>';
            })
    ...
    public function query(TermCondition $model)
    {
        return $model->newQuery()->where('category_id', $this->request()->term_condition_category->id)->latest();
    }

    public function html()
    {
        return $this->builder()
            ->setTableId('term-conditions-table')
    ...
    {
        return 'TermConditions_' . date('YmdHis');
    }
}
