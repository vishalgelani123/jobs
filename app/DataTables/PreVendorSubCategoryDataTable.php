...
class PreVendorSubCategoryDataTable extends DataTable
{
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addIndexColumn()
            ->addColumn('pre_vendor_category', function ($query) {
                return isset($query->preVendorCategory->name) ? $query->preVendorCategory->name : '';
            })
    ...
    public function query(PreVendorSubCategory $model)
    {
        return $model->newQuery()->with('preVendorCategory')->latest();
    }

    public function html()
    {
        return $this->builder()
            ->setTableId('pre-vendor-sub-categories-table')
    ...
    protected function getColumns()
    {
        return [
            Column::computed('DT_RowIndex', 'No')
                ...
                ->searchable(false)
                ->orderable(false),
            Column::make('pre_vendor_category')->title('Pre Vendor Category')->name('preVendorCategory.name')
                ->orderable(false),
    ...
    {
        return 'preVendorSubCategories_' . date('YmdHis');
    }
}
